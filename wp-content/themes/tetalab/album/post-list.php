<div class="info-group">
	...
	<?php if ( bp_has_wire_posts( 'item_id=' . bp_wire_item_id() . '&can_post=' . bp_wire_can_post() ) ) : ?>
		...
		<?php if ( bp_wire_needs_pagination() ) : ?>
			<div id="wire-count" class="pag-count">
				...
				<img id="ajax-loader" src="<?php bp_wire_ajax_loader_src() ?>" height="7" alt="<?php _e( "Loading", "bp-album" ) ?>" style="display: none;" />
			</div>
		...
		<?php while ( bp_wire_posts() ) : bp_the_wire_post(); ?>
			<li>
				<div class="wire-post-metadata">
					...
					<?php printf ( __( 'On %1$s %2$s said:', "bp-album" ), bp_wire_post_date( null, false ), bp_wire_post_author_name( false ) ) ?>
					<?php bp_picture_wire_delete_link() ?>
				</div>
	...
	<?php bp_picture_wire_get_post_form() ?>
</div>
